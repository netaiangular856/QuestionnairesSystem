...
export class RecommendationCreatePanelComponent implements OnInit, OnChanges {
  ...
  readonly aiSuggestBusy = signal(false);
  ...
  private applyAiDraft(d: AiSuggestRecommendationDraftDto): void {
    this.titleAr = d.titleAr ?? '';
    this.titleEn = d.titleEn ?? '';
    this.descriptionAr = d.descriptionAr ?? '';
    this.descriptionEn = d.descriptionEn ?? '';
    const p = d.priority ?? 5;
    this.priorityTier = p <= 4 ? 'low' : p >= 7 ? 'high' : 'medium';
  }
  ...
  suggestWithAi(): void {
    if (this.aiSuggestBusy() || this.submitBusy()) return;
    const surveyId = this.lockedSurveyId ?? this.selectedSurveyId;
    const sid = surveyId?.trim();
    if (!sid) {
      this.toast.show(this.i18n.t('q.rec.aiSurveyRequired'), 'error');
      return;
    }
    this.aiSuggestBusy.set(true);
    this.aiApi.suggestRecommendation({ surveyId: sid }).subscribe({
      next: (d) => {
        this.applyAiDraft(d);
        this.aiSuggestBusy.set(false);
        this.toast.show(this.i18n.t('q.rec.aiFillSuccess'), 'success');
      },
      error: (err: unknown) => {
        this.aiSuggestBusy.set(false);
        const msg =
